...
import { REDIRECT_WITHOUT_TOKEN_TO } from "@/lib/routes";
import { ignoreAsyncFnResult } from "@/lib/utils/generic";

/**
 * Client component that checks search parameters for the `invalidToken` flag that is used by the server to notify that
...
export function InvalidTokenHandler() {
    ...
    useEffect(() => {
        ...
        if (params.has("invalidToken")) ignoreAsyncFnResult(handleInvalidToken)();
    }, [params, router]);
