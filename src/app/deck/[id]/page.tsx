...
export default async function Page({ params }: PageProps) {
    ...
    const now = new Date();
    const preview = (await getDeckPreview(id, now))!; // The isDeckAccessible check also verifies that the deck exists and is non-deleted, and getDeckPreview only returns null for invalid deck IDs, so it is guaranteed to return non-null here
